                ...
                <div class="card mb-2">
                    <div class="card-body" style="background-color: #f7f7f7;">
                        <form method="POST" action="{{route('tambah_komentar_proses', $artikel->id_artikel)}}">
                            ...
                            <?php else: ?>
                                <button type="button" class="btn p-0 pl-2 pr-2 btn-secondary float-right"><small>Kirim</small></button>
                            <?php endif ?>
                        </form>
                    </div>
                </div>
                ...
                <div class="card mb-2">
                    <div class="card-body pt-3">
                        ...
                        <?php if (session("id") == $value->id_user OR session('level') == 1): ?>
                            <a href="<?= route('hapus_komentar_proses', $value->id_komentar ) ?>" onclick="confirm('Apakah anda yakin ingin menghapus?')" class="badge badge-danger float-right">Hapus</a>
                        <?php endif ?>
                        <hr class="m-0">
        ...
        <div class="col-md-3">
            ...
            <?php foreach ($artikel_terbaru as $key => $value): ?>
                <div class="row">
                    ...
                    <div class="col-8">
                        <small><a href="{{route('user_detail_blog', $value->id_artikel, [])}}" style="color:black">{{$value->judul}}</a></small>
                    </div>
                </div>
                <hr>
            <?php endforeach ?>
